function FileManager(cgi_commands) {
	...
	// call cb on success
	this.upload_file_base64 = function(content_base64, target_file_path, cb) {
		let enc_cmd = cgi_commands.encode_command(["eval", "base64", "-d", "-", ">", target_file_path], undefined, undefined, undefined, undefined, true)
		cgi_commands.run_encoded_async(enc_cmd, cb, content_base64)
	}
	...
	this.file_type_to_open_url_gen = {
		text: function(path) {
			return "/static/html/xp/text_editor.html#file_path="+encodeURIComponent(path)
		},
		markdown: function(path) {
			// TODO: Make markdown backend configurable
			return "/static/html/xp/text_editor.html#file_path="+encodeURIComponent(path)
		},
		image: function(path) {
